...
test('V2 migration preserves an existing V1 route', { skip: !runDatabaseTests }, async () => {
  const prisma = new PrismaClient()
  const schema = `qa_v1_${Date.now()}`
  ...
  try {
    ...
    await prisma.$transaction(async tx => {
      ...
      await tx.$executeRawUnsafe('INSERT INTO "LeadChannelRoute" ("id", "organizationId", "sourceKey", "employeeId") VALUES (\'route-v1\', \'org-v1\', \'instagram\', 7)')
      for (const statement of migration.split(';').map(item => item.trim()).filter(Boolean)) {
        await tx.$executeRawUnsafe(statement)
      }
      const rows = await tx.$queryRawUnsafe('SELECT "organizationId", "routeId", "employeeId", "position" FROM "LeadChannelRouteMember"')
      assert.deepEqual(rows, [{ organizationId: 'org-v1', routeId: 'route-v1', employeeId: 7, position: 0 }])
    })
  } finally {
